...
const GITHUB_SETTINGS_KEY = "cctEnglishTrainer.githubSettings.v1";
...
function getProblemTemplateIndex(word) {
  const text = String(word.english || "");
  let total = 0;
...
function buildEnglishTranslationProblem(word) {
  const english = String(word.english || "").trim();
  const index = getProblemTemplateIndex(word);

  const templates = [
    `What does the term "${english}" mean in security and IT?`,
    `Which option best describes "${english}"?`,
    `What is the primary purpose of "${english}"?`,
    `Which statement is true about "${english}"?`,
    `In a security assessment, what does "${english}" refer to?`,
    `Which explanation correctly matches the term "${english}"?`
  ];

  return templates[index];
}

function buildJapaneseTranslationProblem(word) {
  const japanese = String(word.japanese || "").trim();
  const index = getProblemTemplateIndex(word);

  const templates = [
    `セキュリティとITにおいて「${japanese}」という用語は何を意味しますか？`,
    `「${japanese}」を最もよく説明している選択肢はどれですか？`,
    `「${japanese}」の主な目的は何ですか？`,
    `「${japanese}」について正しい説明はどれですか？`,
    `セキュリティ評価において「${japanese}」は何を指しますか？`,
    `「${japanese}」という用語に正しく一致する説明はどれですか？`
  ];

  return templates[index];
}

function buildEnglishSentenceQuestion(word) {
  return buildEnglishTranslationProblem(word);
}

function buildJapaneseExplanation(word) {
  return buildJapaneseTranslationProblem(word);
}
...
function normalizeWord(word) {
  ...
  return {
    ...
    englishQuestion: String(buildEnglishSentenceQuestion(word)).trim(),
    japaneseExplanation: String(buildJapaneseExplanation(word)).trim(),
    englishTranslationProblem: String(buildEnglishTranslationProblem(word)).trim(),
    japaneseTranslationProblem: String(buildJapaneseTranslationProblem(word)).trim(),
...
function createAutoBackup() {
  ...
  updateBackupUI();
  scheduleGitHubAutoSave("auto-backup");
}
...
function getSelectedQuestionMode() {
  ...
  return selected === "translation" ? "translation" : "term";
}

function getQuestionText(word, mode) {
  return mode === "translation" ? word.englishTranslationProblem : word.english;
}

function getCorrectAnswerText(word, mode) {
  return mode === "translation" ? word.japaneseTranslationProblem : word.japanese;
}
...
function renderChoiceQuestion() {
  ...
  $("choiceQuestion").classList.toggle("translation-mode", currentQuestionMode === "translation");
...
function renderWordList() {
  ...
  const filtered = words.filter((word) => {
    const target = normalize(`${word.english} ${word.japanese} ${word.category} ${word.memo} ${word.englishTranslationProblem} ${word.japaneseTranslationProblem}`);
...
function loadGitHubSettings() {
  ...
  const defaults = {
    ...
    repo: "",
    branch: "main",
    path: "data/cct-english-backup.json",
    token: "",
    enabled: false
  };
...
function saveGitHubSettingsFromForm() {
  const settings = {
    ...
    path: $("githubPath")?.value.trim() || "data/cct-english-backup.json",
    token: $("githubToken")?.value.trim() || "",
    enabled: Boolean($("githubAutoSaveEnabled")?.checked)
  };

  localStorage.setItem(GITHUB_SETTINGS_KEY, JSON.stringify(settings));
  updateGitHubStatus(settings.enabled ? "GitHub自動保存：有効" : "GitHub自動保存：無効", "ok");

  if (settings.enabled) {
    scheduleGitHubAutoSave("settings-saved");
  }
}
...
function hydrateGitHubSettingsForm() {
  ...
  if ($("githubOwner")) $("githubOwner").value = settings.owner;
  if ($("githubRepo")) $("githubRepo").value = settings.repo;
  if ($("githubBranch")) $("githubBranch").value = settings.branch;
  if ($("githubPath")) $("githubPath").value = settings.path;
  if ($("githubToken")) $("githubToken").value = settings.token;
  if ($("githubAutoSaveEnabled")) $("githubAutoSaveEnabled").checked = settings.enabled;

  updateGitHubStatus(settings.enabled ? "GitHub自動保存：有効" : "GitHub保存：未設定");
}

function isGitHubConfigured(settings) {
  return Boolean(settings.owner && settings.repo && settings.branch && settings.path && settings.token);
}

function scheduleGitHubAutoSave(reason = "auto") {
  const settings = loadGitHubSettings();

  if (!settings.enabled) return;
  if (!isGitHubConfigured(settings)) {
    updateGitHubStatus("GitHub自動保存：設定が不足しています。Owner / Repository / Branch / 保存先JSON / Token を入力してください。", "error");
    return;
  }

  clearTimeout(githubSaveTimer);
  githubSaveTimer = setTimeout(() => {
    saveBackupToGitHub({ silent: true, reason });
  }, 1800);
}
...
async function getGitHubFileSha(settings) {
  const encodedPath = encodeGitHubPath(settings.path);
  ...
  try {
    const data = await githubFetch(url, { method: "GET" });
...
async function saveBackupToGitHub(options = {}) {
  const settings = loadGitHubSettings();

  if (!isGitHubConfigured(settings)) {
    updateGitHubStatus("GitHub保存：設定が不足しています。", "error");
    return;
  }
  ...
  try {
    ...
    const body = {
      message: `Update CCT English backup ${new Date().toISOString()}`,
      content: toBase64Unicode(backupText),
    ...
    updateGitHubStatus(`GitHub保存：成功（${now}）`, "ok");
  } catch (error) {
    updateGitHubStatus(`GitHub保存：失敗 - ${error.message}`, "error");
    if (!options.silent) alert(`GitHub保存に失敗しました。\n${error.message}`);
  } finally {
    githubSaveInProgress = false;

    if (githubSavePending) {
      githubSavePending = false;
      scheduleGitHubAutoSave("pending");
    }
  }
}
...
async function loadBackupFromGitHub() {
  ...
  if (!isGitHubConfigured(settings)) {
    updateGitHubStatus("GitHub読み込み：設定が不足しています。", "error");
    return;
  }

  if (!confirm("GitHub上のJSONで現在のデータを上書きしますか？")) return;

  try {
    updateGitHubStatus("GitHubから読み込み中...");

    const encodedPath = encodeGitHubPath(settings.path);
    const url = `https://api.github.com/repos/${encodeURIComponent(settings.owner)}/${encodeURIComponent(settings.repo)}/contents/${encodedPath}?ref=${encodeURIComponent(settings.branch)}`;
    const data = await githubFetch(url, { method: "GET" });

    if (!data?.content) throw new Error("GitHub上のファイル内容を取得できませんでした。");

    const backupText = fromBase64Unicode(data.content);
    const imported = JSON.parse(backupText);
    const importedWords = Array.isArray(imported) ? imported : imported.words;

    if (!Array.isArray(importedWords)) {
      throw new Error("JSON内にwords配列がありません。");
    }
    ...
    updateGitHubStatus(`GitHub読み込み：成功（${new Date().toLocaleString("ja-JP")}）`, "ok");
  } catch (error) {
    updateGitHubStatus(`GitHub読み込み：失敗 - ${error.message}`, "error");
...
function bindGitHubEvents() {
  ...
  $("saveGithubSettingsBtn")?.addEventListener("click", saveGitHubSettingsFromForm);
  $("saveGithubNowBtn")?.addEventListener("click", () => {
    saveGitHubSettingsFromForm();
    saveBackupToGitHub({ silent: false, reason: "manual" });
  });
  $("loadGithubBackupBtn")?.addEventListener("click", () => {
    saveGitHubSettingsFromForm();
    loadBackupFromGitHub();
  });
}
