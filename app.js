...
function buildAnswerMessage(word, correct) {
  ...
  return `不正解。正解は「${currentWord.japanese}」です。連続正解は0 / ${MASTER_STREAK}に戻りました。`;
}
...
function handleChoice(event) {
  ...
  const correct = selected === currentWord.japanese;

  document.querySelectorAll(".choice-option").forEach((option) => {
    option.disabled = true;
    if (option.dataset.answer === currentWord.japanese) option.classList.add("correct");
